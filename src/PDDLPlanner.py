...
class PDDLPlanner:
    """
    Genera PDDL e risolve il problema tramite un planner esterno
    (la connessione al planner è delegata a PlannerConnection)
    """
    ...
    def solve(self):
        # 1. genera i file PDDL nella cartella benchmarks
        self._write_pddl()

        # 2. chiama il planner tramite la connessione
        plan = self.connection.run("domain.pddl", "problem.pddl")
        return plan

    def _write_pddl(self):
        """Genera domain.pddl e problem.pddl a partire dal livello Sokoban nella cartella benchmarks"""
        ...
        for r, row in enumerate(self.level):
            for c, char in enumerate(row):
                if char != '#': positions.append(f"p{r}_{c}")
                if char == '#': walls.add((r,c))
                if char == '$': boxes.append((r,c))
                if char == '.': goals.append((r,c))
                if char == '@': player = (r,c)

        # Scrive i file nella cartella benchmarks
 ...
 (:action push-right
  ...
  :effect (and (player-at ?p ?t)(not (player-at ?p ?f))(box-at ?b ?n)(not (box-at ?b ?t))(empty ?f)(not (empty ?n)))
        ...
        with open(problem_path,"w") as f:
            ...
            for i in range(len(boxes)): f.write(f" box{i} - box\n")
            ...
            for pos in positions:
                ...
                if (r,c) not in occ: f.write(f" (empty {pos})\n")
                if (r-1,c) not in walls and r>0: f.write(f" (adj-up p{r}_{c} p{r-1}_{c})\n")
                if (r+1,c) not in walls and r<rows-1: f.write(f" (adj-down p{r}_{c} p{r+1}_{c})\n")
                if (r,c-1) not in walls and c>0: f.write(f" (adj-left p{r}_{c} p{r}_{c-1})\n")
                if (r,c+1) not in walls and c<cols-1: f.write(f" (adj-right p{r}_{c} p{r}_{c+1})\n")
            f.write(" )\n (:goal (and\n")
            for i in range(len(boxes)): f.write(f" (box-at box{i} p{goals[i][0]}_{goals[i][1]})\n")
            f.write(" ))\n)")
